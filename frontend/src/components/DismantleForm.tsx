...
export default function DismantleForm({ userName, userPermissions, onBack }: DismantleFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!reason) {
      setError('Zadejte prosím důvod demontáže.');
      return;
    }
    ...
    try {
      ...
      formData.append('action', 'Demontáž z linky');
  ...
  if (isScanning) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center animate-fade-in-up">
        <h2 className="text-2xl font-bold mb-4">Demontáž dílu</h2>
        <p className="text-gray-500 mb-8">Naskenujte kód dílu, který byl demontován.</p>
  ...
  if (isSuccess) return (
    <div className="w-full max-w-md mx-auto text-center p-12 bg-white dark:bg-gray-800 rounded-3xl mt-12 border animate-pop-in">
      ...
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Úspěšně demontováno</h2>
    </div>
  );

  return (
    <PartSplitLayout part={part} title="Demontáž" onBack={onBack}>
      <form onSubmit={handleSubmit} className="flex flex-col h-full space-y-6">
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Důvody odložení</h3>
          {error && <div className="p-3 mb-4 bg-red-50 text-red-600 rounded-xl text-sm">{error}</div>}
          
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Datum a čas odložení</label>
              <input type="datetime-local" value={date} onChange={e => setDate(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Důvod demontáže</label>
              <textarea value={reason} onChange={e => setReason(e.target.value)} rows={3} className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Závada, údržba..."></textarea>
            </div>

            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Fotografie závady</label>
        ...
        <div className="mt-auto pt-6 border-t border-gray-100 dark:border-gray-700 flex justify-end">
          <button type="submit" disabled={isSubmitting} className="flex items-center gap-2 px-8 py-3 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-orange-500/30 disabled:opacity-50">
            {isSubmitting ? 'Ukládám...' : 'Potvrdit demontáž'}
